...
GrowthCone_SelfReferentialForces::GrowthCone_SelfReferentialForces(const GrowthCone_SelfReferentialForces &copy)
    : GrowthCone(copy)
...
GCPtr GrowthCone_SelfReferentialForces::clone(BaseWeakNodePtr parent, NeuritePtr neurite,
        ...
        double &substep, double &new_angle)
{
    ...
    new_center.shift(somatropic_.force * pow(distanceToSoma, -somatropic_.decay), soma_angle);
    ...
    new_center = Point(new_center[0]*scale_factor, new_center[1]*scale_factor);

    double sigma_angle = sqrt(substep)*move_.sigma_angle;

    double x = generateGaussianNoise(new_center[0], scale_factor*sigma_angle);
    double y = generateGaussianNoise(new_center[1], scale_factor*scale_factor*sigma_angle);

    // compute the angle and fallback to polar coordinate system already implemented
    new_angle = atan2(y,x);
